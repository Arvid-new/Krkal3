...
CKerName::CKerName(const char *Name, eKerNameType type, CKerNamesMain *NamesMain){
...
char * CKerName::ToShortString() {
	return KerNamesMain->KerMain->KerServices->ConverNameToShort(name); 
}
...
// Konstruktor
CKerNamesMain::CKerNamesMain(CKerMain *kerMain) {
	KerMain = kerMain;
	Names = 0;
	NameInCycle = 0;
	MatrixesCalculated = 0;
...
CKerNameList *CKerNamesMain::FindLayer(CKerName *From, int (*FilterFce)(CKerName *name), int Type , int direction  ) {
	...
	while (list) {
		...
		if (direction==1) {
			l = l2->name->childs; 
			if (l2->name == KerMain->KnownNamesPtrs[eKKNeverything]) l = Names;
		} else l = l2->name->parents;
		while (l) {
			if (l->name != KerMain->KnownNamesPtrs[eKKNeverything]) {
				if (TEST(l->name)) FindLayerOutput(l->name,&out,direction);
				else list = new CKerNameList(l->name,list);
			}
			l = l->next;
...
CKerNameList *CKerNamesMain::FindSet(CKerName *From, int (*FilterFce)(CKerName *name), int Type, int direction ) {
	...
	CKerNameList *out = 0;
	while (list) {
		if (!From || (direction==1 && KerMain->KerServices->IsFMemberOfS(list->name,From)) || (direction != 1 && KerMain->KerServices->IsFMemberOfS(From,list->name))) {
