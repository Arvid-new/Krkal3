...
int UnicodeToAnsi(char* dest, int destBuffSize, const wchar_t *source) {
	return WideCharToMultiByte(CP_ACP, 0, source, -1, dest, destBuffSize, 0, 0);
}
...
char *UnicodeToAnsi(const wchar_t *source) {
	if (!source)
		return 0;
	int size = WideCharToMultiByte(CP_ACP, 0, source, -1, 0, 0, 0, 0);
	char *ret = new char[size];
	WideCharToMultiByte(CP_ACP, 0, source, -1, ret, size, 0, 0);
	return ret;
}

int AnsiToUnicode(wchar_t *dest, int destBuffSizeIn2B, const char *source) {
	return MultiByteToWideChar(CP_ACP, 0, source, -1, dest, destBuffSizeIn2B);
}
...
wchar_t* AnsiToUnicode(const char *source) {
	if (!source)
		return 0;
	int size = MultiByteToWideChar(CP_ACP, 0, source, -1, 0, 0);
	wchar_t *ret = new wchar_t[size];
	MultiByteToWideChar(CP_ACP, 0, source, -1, ret, size);
	return ret;
}


void KernelFreeArray(void *pointer) {
	delete[] pointer;
}
...
CKerName *CQueryKsParameters::GetKsidName(const char *name) { 
	CKerName *ret = KerMain->KerNamesMain->GetNamePointer(name);
	if (!ret)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound, 0, name);
	return ret;
}

void *CQueryKsParameters::GetStaticVariablePtr(CKerName *varName) { 
	void **s = KerMain->StaticData;
	CKerOVar *var = KerMain->Static->ObjectType->GetVariable(varName);
	if (!var)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound, 0, varName->name);
	return (UC*)s + var->DataOffset;
}

int CQueryKsParameters::GetVariableOffset(CKerName *obj, CKerName *varName) { 
	if (!varName || !obj || obj->Type != eKerNTclass)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound);
	CKerOVar *var = obj->ObjectType->GetVariable(varName);
	if (!var)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound, 0, varName->name);
	return var->Offset;
}

int CQueryKsParameters::GetCastOffset(CKerName *fromDerivedObj, CKerName *toBaseObj) { 
	if (!fromDerivedObj || !toBaseObj || fromDerivedObj->Type != eKerNTclass)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound);
	int ret = fromDerivedObj->ObjectType->GetBaseObjOffset(toBaseObj);
	if (ret == -1)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound, 0, toBaseObj->name);
	return ret;
}

void (* CQueryKsParameters::GetDirectMethodPtr(CKerName *methodName))() { 
	if (!methodName || (methodName->Type != eKerNTdirectMethod && methodName->Type != eKerNTstaticDirectMethod))
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound);
	void (*ret)() = methodName->DirectMethodPtr;
	if (!ret)
		KerMain->Errors->LogError(eKRTEfscriptEntryPointnotFound, 0, methodName->name);
	return ret; 
}
...
CKerServices::CKerServices(CKerMain *kerMain)
{
	/*IntBuff = 0; ObjBuff = 0;*/ QuickSave = 0; KerMain = kerMain; _backend=0;
	_backend = new CKerServicesBackend();
}

CKerServices::~CKerServices() {
...
int CKerServices::IsFMemberOfS(CKerName *first, CKerName *second) {
	...
	if (second == KerMain->KnownNamesPtrs[eKKNeverything] && first != KerMain->KnownNamesPtrs[eKKNeverything]) return 1;
...
int CKerServices::IsFMemberOfS(const OPointer &first_obj, CKerName *second) {
	if (!second) return 0;
	CKerObject *kobj = first_obj.KerObject();
	CKerName *first;
...
///////////////////////////////////////////////////////////////////
// Vrati, jedna, jestlize jmena existuji a prvni <= Druhemu; JInak nulu
int CKerServices::IsFMemberOfS(const OPointer &first_obj, const OPointer &second_obj) {
	CKerName *first;
	CKerName *second;
	CKerObject *kobj = first_obj.KerObject();
	if (!kobj||!(first = kobj->Type->Name)) return 0;
	kobj = second_obj.KerObject();
...
///////////////////////////////////////////////////////////////////
// Vrati, jedna, jestlize jmena existuji a prvni <= Druhemu; JInak nulu
int CKerServices::IsFMemberOfS(CKerName *first, const OPointer &second_obj) {
	if (!first) return 0;
	CKerName *second;
	CKerObject *kobj = second_obj.KerObject();
	if (!kobj||!(second = kobj->Type->Name)) return 0;
	if (first==second) return 1;
	if (second == KerMain->KnownNamesPtrs[eKKNeverything] && first != KerMain->KnownNamesPtrs[eKKNeverything]) return 1;
	if (first->Compare(second)>=2) return 1;
...
// Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu
// Prvni jmeno si zjisti jako typ objektu
int CKerServices::IsFMemberOfSExcl(const OPointer &first_obj, CKerName *second) {
	if (!second) return 0;
	CKerObject *kobj = first_obj.KerObject();
	CKerName *first;
	if (!kobj||!(first = kobj->Type->Name)) return 0;
...
///////////////////////////////////////////////////////////////////
// Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu
int CKerServices::IsFMemberOfSExcl(const OPointer &first_obj, const OPointer &second_obj) {
	CKerName *first;
	CKerName *second;
	CKerObject *kobj = first_obj.KerObject();
	if (!kobj||!(first = kobj->Type->Name)) return 0;
...
///////////////////////////////////////////////////////////////////
// Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu
int CKerServices::IsFMemberOfSExcl(CKerName *first, const OPointer &second_obj) {
	if (!first) return 0;
	CKerName *second;
	CKerObject *kobj = second_obj.KerObject();
	if (!kobj||!(second = kobj->Type->Name)) return 0;
...
bool CKerServices::RecursiveArrayCompare(CLT lt, CKerArrBase* arr1, CKerArrBase* arr2) {
	if (arr1 == arr2)
		return true;
	if (!arr1 || !arr2 || arr1->GetCount() != arr2->GetCount())
		return false;
	if (lt.DimCount == 1) {
		switch (lt.Type) {
			case eKTchar:
				return ((CKerArr<wchar_t>*)arr1)->Compare((CKerArr<wchar_t>*)arr2) == 0;
			case eKTdouble:
				return ((CKerArr<double>*)arr1)->Compare((CKerArr<double>*)arr2) == 0;
			case eKTint:
				return ((CKerArr<int>*)arr1)->Compare((CKerArr<int>*)arr2) == 0;
			case eKTname:
			case eKTobject:
				return ((CKerArr<void*>*)arr1)->Compare((CKerArr<void*>*)arr2) == 0;
			default:
				assert(false);
				return false;
		}
	} else {
		for (int f=0; f < arr1->GetCount(); f++) {
			if (!RecursiveArrayCompare(CLT(lt.Type, lt.Modifier, lt.ObjectType, lt.DimCount-1), (*(ArrPtr<int, 2>*)&arr1)[f], (*(ArrPtr<int, 2>*)&arr1)[f]))
				return false;
		}
		return true;
	}
...
CKerName *CKerServices::CreateName(eKerNameType type, CKerName *localizeTo, const wchar_t *identifier, const wchar_t *version) {
	if (!localizeTo || !identifier || !*identifier)
		return 0;

	// check identifier validity
	char buff[512];
	int f;
	if (wcsstr(identifier, L"__M_"))
		return 0;
	for (f=0; identifier[f]; f++) {
		int ch = identifier[f];
		if (ch > 127 || f >= sizeof(buff)-1)
			return 0;
	...
	CKerName *ret = KerMain->KerNamesMain->GetNamePointer(str.c_str());
	if (ret)
		return ret;

	ret = new CKerName(str.c_str(), type == eKerNTclass ? eKerNTundefinedClass : type, KerMain->KerNamesMain);
	_backend->NewNames.push_back(ret);
	return ret;
}



CKerName *CKerServices::CreateName(eKerNameType type, const char *ksid, const CLT &lt) {
	if (!ksid || !*ksid)
		return 0;

	CKerName *ret = KerMain->KerNamesMain->GetNamePointer(ksid);
	if (ret)
		return ret;

	ret = new CKerName(ksid, type == eKerNTclass ? eKerNTundefinedClass : type, KerMain->KerNamesMain);
	ret->LT = lt;
	_backend->NewNames.push_back(ret);
	return ret;
...
void CKerServices::CreateDependency(CKerName *parent, CKerName *child) {
	if (!parent || !child)
		return;
	parent->AddChild(child);
	_backend->NewDependencies.push_back(parent);
	_backend->NewDependencies.push_back(child);
}




bool CKerServices::CreateDataObject(const OPointer &obj, CKerName *name) {
	if (!obj || !name)
		return false;
	CKerObject *o = obj.KerObject();
	if (o->DataObjectName == name)
		return true;
	if (o->DataObjectName || name->DataObject)
		return false;

	new CDataObject(name, o, KerMain);
	_backend->NewDataObjects.push_back(name);
...
bool CKerServices::CreateDataObject(CKerName *className, CKerName *name) {
	if (!className || !name)
		return false;
	if (!KerMain->IsDataOnlyMode())
		return false;
	if (!name->DataObject) {
...
void CKerServices::ForgetAllNewEntities() {
	_backend->ForgetAllNewEntities();
}



KString CKerServices::GetDataFileFromCodeFile(eKerLacalizeDataFileTo localizeTo) {
	const char * code = KerMain->KernelParameters->GetCodeFile();
	if (!code || !*code)
		return 0;
	for (const char *f = code; *f; f++) {
		if (*f == '/' || *f == '\\')
			code = f+1;
	}
	if (!*code)
	...
	string str;
	switch (localizeTo) {
		case eKLDFTDataFiles:
			str = "$DATAFILES$\\";
			break;
		case eKLDFTRootFiles:
			str = "$ROOTFILES$\\";
			break;
		case eKLDFTProfile:
		;// TODO
	}
	str.append(code, strlen(code) - 5);
	str.append(".data");
...
int CKerServices::Random(int range) {
	return _backend->mtr.randInt(range);
}
...
//	if (!ObjType || ObjType->Type != eKerNTclass) return 0; else return ObjType->ObjectType->DefAuto; 
//}
//
//
//////////////////////////////////////////////////////////////////
...
__int64 CKerServices::GenerateVersionNumber(char *DestStr ) {
	__int64 a,b,c,d;
	a = _backend->mtr.randInt(0xFFFF);
	b = _backend->mtr.randInt(0xFFFF);
	c = _backend->mtr.randInt(0xFFFF);
	d = _backend->mtr.randInt(0xFFFF);
	if (DestStr) {
