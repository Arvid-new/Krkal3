...
KRKALRUNTIME_API int UnicodeToAnsi(char* dest, int destBuffSize, const wchar_t *source);
KRKALRUNTIME_API char *UnicodeToAnsi(const wchar_t *source);
KRKALRUNTIME_API int AnsiToUnicode(wchar_t *dest, int destBuffSizeIn2B, const char *source);
KRKALRUNTIME_API wchar_t* AnsiToUnicode(const char *source);
KRKALRUNTIME_API void KernelFreeArray(void *pointer);
...
inline KString ToString(double d) {
	wchar_t buffer[32];
	swprintf_s(buffer, L"%g", d);
	return KString(buffer);
}

inline KString ToString(int a) {
	wchar_t buffer[32];
	swprintf_s(buffer, L"%i", a);
	return KString(buffer);
}
...
class KRKALRUNTIME_API KsInterface {
public:
	virtual void Initialize(CKerMain *kerMain, void *servicesHandle) = 0;
	virtual void (* GetMethodPointer(const char *methodKsf))() = 0;
	virtual void (* GetInitializationPointer(CKerName *objectName))() = 0;
	virtual void Destruct() = 0;
};


class KRKALRUNTIME_API CQueryKsParameters {
public:
	CQueryKsParameters(CKerMain *kerMain) {
		KerMain = kerMain;
	}
	CKerName *GetKsidName(const char *name);
	void *GetStaticVariablePtr(CKerName *varName);
	int GetVariableOffset(CKerName *obj, CKerName *varName);
	int GetCastOffset(CKerName *fromDerivedObj, CKerName *toBaseObj);
	void (* GetDirectMethodPtr(CKerName *methodName))();
private:
	CKerMain *KerMain;
};
...
class KRKALRUNTIME_API CKerServices {
public:
	CKerServices(CKerMain *kerMain);
	~CKerServices();
	...
	int IsFMemberOfS(const OPointer &first_obj, CKerName *second); // Vrati, jedna, jestlize jmena existuji a prvni <= Druhemu; JInak nulu.  Prvni jmeno si zjisti jako typ objektu
	int IsFMemberOfS(CKerName *first, const OPointer &second_obj); // Vrati, jedna, jestlize jmena existuji a prvni <= Druhemu; JInak nulu
	int IsFMemberOfS(const OPointer &first_obj, const OPointer &second_obj); // Vrati, jedna, jestlize jmena existuji a prvni <= Druhemu; JInak nulu. 
	int IsFMemberOfSExcl(CKerName *first, CKerName *second); // Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu
	int IsFMemberOfSExcl(const OPointer &first_obj, CKerName *second); // Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu.  Prvni jmeno si zjisti jako typ objektu
	int IsFMemberOfSExcl(CKerName *first, const OPointer &second_obj); // Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu
	int IsFMemberOfSExcl(const OPointer &first_obj, const OPointer &second_obj); // Vrati, jedna, jestlize jmena existuji a prvni < Druhemu; JInak nulu. 

	static bool RecursiveArrayCompare(CLT lt, CKerArrBase* arr1, CKerArrBase* arr2);


	CKerName *CreateName(eKerNameType type, CKerName *localizeTo, const wchar_t *identifier, const wchar_t *version);
	CKerName *CreateName(eKerNameType type, const char *ksid, const CLT &lt);
	void CreateDependency(CKerName *parent, CKerName *child);
	bool CreateDataObject(const OPointer &obj, CKerName *name);
	bool CreateDataObject(CKerName *className, CKerName *name);
	void ForgetAllNewEntities();
	KString GetDataFileFromCodeFile(eKerLacalizeDataFileTo localizeTo);
