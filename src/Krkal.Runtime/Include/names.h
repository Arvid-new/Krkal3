...
class KRKALRUNTIME_API CKerNamesMain {
friend CKerName;
friend CKerCalculatePHT;
public:
	CKerNamesMain(CKerMain *kerMain);
	~CKerNamesMain();
	...
	CKerName * GetNamePointer(const char *name) {return (CKerName*)HashTable->Member(name);} // 0 - error nezname jmeno
	...
	int GetNamesCount() {return _namesCount; }
	CKerMain *KerMain;
private:
...
class KRKALRUNTIME_API CKerName : public CHashElem {
friend CKerNamesMain;
friend class CKerSaver;
public:
	CKerName(const char *Name, eKerNameType type, CKerNamesMain *NamesMain);
	virtual ~CKerName();
	...
	union {								// Dodatecna informace ke jmenu zavisla podle typu jmena
		...
		class CKerObjectT *ObjectType;	// Typ objektu. Pro objekty
		void (*DirectMethodPtr)();		// pointer to direct method function
	};
	CDataObject *DataObject;
	CLT LT;	// language type;
	char * GetNameString() { return name; }
	char * ToString() { return name; }
	char * ToShortString();  // vrati kratke jmeno, zbavene verzi, _KSID_ ... Fce pro string pouziva svuj buffer, nedealokovat
	CAttributes *Attributes;
//	char * GetUserName() { if (UserName) return UserName; else return ToShortString(); } // vrati uzivatelsky jmeno, pokud neni vrati kratke jmeno. vraceny string nemenit, nedealokovat
