...
CKerErrorLog::CKerErrorLog(CKerMain *KerMain) {
	stack = 0;
	next=0; prev=0;
	nextig=0;
...
inline void CKerErrors::LogError2(int Line, int errornum, int errorparam, const char *errorstr) {
	if (KerMain->KerContext) KerMain->KerContext->line = Line;
...
// VYTVORENI LOGU
void CKerErrors::LogError( int errornum, int errorparam, const char *errorstr) {
	...
	if (Logs[eg].max) { // loguju:
		...
		if (KerMain->KerContext) log->stack = new CKerErrorStack(KerMain->KerContext,0);

		KerMain->KernelParameters->CallLoggingCB(log->time, errornum, errorparam, log->ErrorStr);
	}
...
CKerErrors::CKerErrors(CKerMain *kerMain, eKerDebugModes DebugMode) {
	KerMain = kerMain;
	int f;
	start = 0; end = 0;
	//LogWindow = 0;
