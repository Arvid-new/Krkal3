...
int FSRTypeSizes[FSRNumTypes] = {1,4,8,8,1,1,2,2};
...
CFSRegKey::CFSRegKey(CFSRegister *reg,char Type, const char *Name, CFSRegister *SubReg) {
	next = 0;
...
void CFSRegKey::rename(const char *Name) {
	if (MyRegister->HashTable) {
		MyRegister->HashTable->NumKeys--;
		HFindAndDelete(&(MyRegister->HashTable->HT[MyRegister->HashTable->HashFunction(name,MyRegister)]),this);
	}
...
void CFSRegKey::write64(__int64 a) {
...
void CFSRegKey::writew(wchar_t a) {
	assert(type==FSRTwChar||type==FSRTwString);
	if (pos>=size2) if (!resize(pos+1)) return;
	if (pos>top) top=pos;
	((wchar_t*)data)[pos] = a;
	pos++;
...
__int64 CFSRegKey::read64() {
	assert(type==FSRTint64);
	if (pos>=top||pos>=size2) return 0;
	pos++;
	return ((__int64*)data)[pos-1];
}
wchar_t CFSRegKey::readw() {
	assert(type==FSRTwChar||type==FSRTwString);
	if (pos>=top||pos>=size2) return 0;
	pos++;
	return ((wchar_t*)data)[pos-1];
}
...
wchar_t * CFSRegKey::wstringread() {
	assert(type==FSRTwChar||type==FSRTwString);
	wchar_t *string;
	int oldpos=pos;
	int f;
	if (pos>=top) return 0;
	while(pos<size2&&((wchar_t*)data)[pos]) pos++;
	string = new wchar_t[pos-oldpos+1];
	for(f=oldpos;f<pos;f++) string[f-oldpos] = ((wchar_t*)data)[f];
	string[pos-oldpos]=0;
	pos++;
	return string;
}
...
void CFSRegKey::SetPosToNextString() {
	...
	switch (type) {
		...
		case FSRTdouble:
			...
			break;
		case FSRTint64:
			while (pos<size2&&pos<top&&((__int64*)data)[pos]) pos++;
			break;
		case FSRTwChar:
		case FSRTwString:
			while (pos<size2&&pos<top&&((wchar_t*)data)[pos]) pos++;
			break;
	}
...
char * CFSRegKey::GetDirectAccessFromPos() { return data+pos*FSRTypeSizes[type]; } // vrati ukazatel na aktualni pozici v datech (Po resize se musi volat znova)
...
CFSRegKey *CFSRegister::FindKey(const char *name) {if (!HashTable) CreateHT(); return HashTable->Member(name,this);} // vyhleda klic pogle jmena

CFSRegKey *CFSRegister::FindNextKey(const char *name,CFSRegKey *PrevKey) {if (!HashTable) CreateHT(); return HashTable->Member(name,this,PrevKey);} // vyhleda nasledujici klic podle jmena

void CFSRegister::SetRegisterToBeCompressed() { if (RootInfo) RootInfo->CompressMode = 1;}
void CFSRegister::SetRegisterToBeUnCompressed() {  if (RootInfo) RootInfo->CompressMode = 0;}

const char* CFSRegister::GetHeader() {
	return RootInfo ? RootInfo->Head : 0;
}
...
int CFSRegister::VerifyRegisterFile(const char *path, const char *head) {
...
bool CFSRegister::CheckVersionFeatures() {
	assert(RootInfo);
	return FS->CheckVersionFeatures(this);
}
...
void CFSRegister::WriteVersionFeatures() {
	assert(RootInfo);
	FS->WriteVersionFeatures(this);
}
