...
namespace Puzzle.Windows.Forms.SyntaxBox
{
	...
	public class EditViewControl : SplitViewChildControl
	{
		...
		public void InitVars()
		{
			...
			_View.FirstVisibleColumn = hScroll.Value;
			_View.FirstVisibleRow = vScroll.Value;
			//	View.yOffset =_yOffset;

			_View.VisibleRowCount = 0;
			if (hScroll.Visible)
				_View.VisibleRowCount = (this.Height - hScroll.Height) / View.RowHeight
					+ 1;
			else
				_View.VisibleRowCount = (this.Height - hScroll.Height) / View.RowHeight
					+ 2;
			...
			if (this.ShowGutterMargin)
				_View.GutterMarginWidth = this.GutterMarginWidth;
			else
				_View.GutterMarginWidth = 0;
			...
			if (this.ShowLineNumbers)
			{
				...
				_View.LineNumberMarginWidth = 10 + this.Painter.MeasureString(s).Width;
			}
			else
				_View.LineNumberMarginWidth = 0;


			_View.TotalMarginWidth = View.GutterMarginWidth +
				View.LineNumberMarginWidth;
			if (Document.Folding)
				_View.TextMargin = View.TotalMarginWidth + 20;
			else
				_View.TextMargin = View.TotalMarginWidth + 7;


			_View.ClientAreaWidth = this.Width - vScroll.Width - View.TextMargin;
			_View.ClientAreaStart = View.FirstVisibleColumn * View.CharWidth;
		}
						...
						if (r2.Expansion_StartSegment != null)
						{
								...
								r2.Expansion_StartSegment.Expanded == false)
							{
								if (!IsOverSelection(e.X, e.Y))
								{
									...
									this.Redraw();
									_View.Action = XTextAction.xtSelect;
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			_View.Action = XTextAction.xtNone;
			base.OnMouseUp(e);
		}
			...
			drgevent)
		{
			if (!this.ReadOnly)
			{
				if (Document != null)
				{
					_View.Action = XTextAction.xtDragText;
		                                 ...
		                                 IntelliMouse.ScrollEventArgs e)
		{
			...
			this._View.YOffset = -(int)_IntelliScrollPos;
